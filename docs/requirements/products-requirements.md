...
| Description        | Yes (assumed) | Rich text via `packages/text-editor` (Tiptap, JSON), consistent with Articles/Case Studies                                                                                                                                                                                                                                                                                                  |
| Excerpt            | No            | Optional textarea, ~200 chars soft-recommended. Powers the OG meta description on share; falls back to auto-truncated plain text from Description if left blank. Same field/logic as Articles/Case Studies — see `text-editor-requirements.md` § Excerpt/Summary field.                                                                                                                     |
| Gallery            | No            | Multiple image upload, via **Media Library**                                                                                                                                                                                                                                                                                                                                                |
| Product URL / Link | No            | Live product link                                                                                                                                                                                                                                                                                                                                                                           |
| Stage              | Yes           | Fixed enum: `IDEA` / `MVP` / `BETA` / `LIVE` / `PAUSED` / `SUNSET`. `PAUSED` and `SUNSET` are both terminal-ish but distinct: `PAUSED` implies temporarily inactive (may resume later), `SUNSET` implies permanently discontinued. Product creator sets/changes Stage the same as any other field on their own listing — no special transition restrictions or one-way lock between stages. |
...
- **Rich Text Editor** (`text-editor-requirements.md`, package `packages/text-editor`) — Description field; also the source of the Excerpt field's fallback logic.
- **Media Library** (`packages/media-library`) — Banner, Gallery.
- **Global Commenting** (`global-commenting-requirements.md`, package `packages/comments`) — comments/replies on the product.
